  ...
  return useQuery({
    ...
    queryFn: async (): Promise<FinsDataPayload> => {
      ...
      const [
        ...
        client.from("fins_watchlists").select("id, name, status, updated_at").eq("status", "active").order("updated_at", { ascending: false }).limit(4),
        client.from("fins_watchlist_companies").select("id, ticker, company_name, sector, priority_tier").order("priority_tier", { ascending: true }).order("ticker", { ascending: true }).limit(24),
        client.from("fins_disclosure_events").select("id, ticker, filing_type, source_type, title, event_timestamp, source_url, period_end").order("event_timestamp", { ascending: false }).limit(200),
        client.from("fins_fused_signals").select("id, disclosure_event_id, ticker, directional_sentiment, confidence, causal_summary, comparative_context, created_at").order("created_at", { ascending: false }).limit(200),
        client.from("fins_decision_records").select("id, disclosure_event_id, action").order("created_at", { ascending: false }).limit(200),
        client.from("fins_signal_evidence").select("id, disclosure_event_id, snippet").order("created_at", { ascending: false }).limit(300),
        client.from("fins_alerts").select("id, disclosure_event_id, severity, message, created_at").order("created_at", { ascending: false }).limit(20),
